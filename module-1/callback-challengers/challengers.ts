...
*/
const addS = (value: string) => value + "s";
...
// map() takes two argument number[] and callback().
// foreach item in arr run function addTwo() return a list of number + 2
const map = (arr: number[], callback: (num: number) => number): number[] => {
	const result: number[] = [];
	for (const item of arr) {
		// for all item in arr
		result.push(callback(item)); // calls addTwo() returns item + 2
	}
	return result; // return a new arr[]
};

// map2 changes the original arr. No return value.
const map2 = (arr: number[], callback: (num: number) => number): void => {
...
// Using generic type T to accept any type
const forEach = <T>(arr: T[], callback: (item: T) => void): void => {
	for (const item of arr) {
		callback(item);
	}
};

forEach(letters, (char: string): void => {
	alphabet += char;
});
console.log(alphabet); // Output: abcd

// A more simple way to do the same
// forEach method runs a callback for each array element – no return value
let myString = "";
...
console.log(mapWith([5, 7, 9], addTwo)); // should output [ 7, 9, 11 ]
